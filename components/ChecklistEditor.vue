<template>
  <div class="space-y-3">
    <div class="flex items-center justify-between mb-4">
      <div class="text-sm text-gray-400 uppercase tracking-wide">Items</div>
      <button @click="add" class="text-xs text-indigo-300 font-bold hover:text-white transition-colors bg-white/5 px-2 py-1 rounded hover:bg-white/10">+ ADD ITEM</button>
    </div>
    ...
    <div 
      v-for="item in items" 
      :key="item.id" 
      class="flex items-start gap-3 p-3 bg-white/5 border border-white/5 rounded-xl group hover:bg-white/10 hover:border-white/10 transition-all duration-300"
    >
      <!-- State Toggle -->
      <button 
      ...
      <div class="flex-1 space-y-1">
        ...
        <input 
          ...
          class="block w-full text-sm font-medium dark:text-white text-gray-900 placeholder-gray-500 bg-transparent border-none p-0 focus:ring-0 transition-colors"
          placeholder="Item name (e.g. CV)"
        />
        <!-- Link -->
        <div class="flex items-center gap-2 group/link">
           <svg class="w-3 h-3 text-indigo-400 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
           <input 
             v-model="item.link" 
             @blur="save(item)"
             @keyup.enter="($event.target as HTMLInputElement).blur()"
             class="block w-full text-xs text-indigo-500 dark:text-indigo-300 placeholder-gray-400 dark:placeholder-gray-600 bg-transparent border-none p-0 focus:ring-0 transition-colors"
             placeholder="Add link..."
           />
           <a v-if="item.link" :href="item.link" target="_blank" class="opacity-0 group-hover/link:opacity-100 text-indigo-500 dark:text-indigo-300 hover:text-white transition-opacity">
      ...
      <!-- Delete -->
      <button @click="remove(item.id)" class="text-gray-400 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all scale-90 group-hover:scale-100">
...
const { checklistItems, upsertChecklistItem, deleteChecklistItem } = useApplications();

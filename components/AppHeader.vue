    ...
    <div class="container mx-auto px-6 h-16 flex justify-between items-center">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3 pl-0 md:pl-6 md:border-l border-brand-dark/10 dark:border-brand-light/10 h-8">
           ...
           <!-- Calendar Export -->
           <button @click="exportCalendar" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-brand-dark/10 dark:hover:bg-brand-light/10 transition-colors text-brand-dark/60 hover:text-brand-dark dark:text-brand-light/60 dark:hover:text-brand-light" title="Export to Calendar">
...
const exportCalendar = () => {
    ...
    useConfirm().ask('Download deadlines for your calendar?', 'Export Calendar').then(confirmed => {
        if (confirmed) {
            downloadCalendar(applications.value);
        }
    });
};
